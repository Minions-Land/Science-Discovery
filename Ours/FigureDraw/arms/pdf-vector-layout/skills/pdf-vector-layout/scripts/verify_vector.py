    ...
    try:
        ...
        for label in expected_labels:
            ...
            if count == 0:
                ...
                issues.append(f"label '{label}' appears {count}× (possible duplicate)")
        if issues:
...
def render_preview(pdf_path, dpi=150):
    ...
    try:
        subprocess.run(
            [pdftocairo, '-png', '-f', '1', '-l', '1', '-r', str(dpi),
             str(pdf_path), str(preview_base)],
            capture_output=True,
            timeout=60,
        )
...
def check_text_duplication(pdf_path, source_pdf):
    ...
    try:
        ...
        duplicated = {tok: out_tokens[tok] - src_tokens[tok]
        ...
        return True, f"No tokens duplicated vs source"
    except Exception as e:
...
def main():
    ...
    parser.add_argument('--expected-labels', nargs='*', default=[],
                        help="Panel labels expected to appear (e.g. f g h i j)")
    parser.add_argument('--source', type=Path, default=None,
                        help="Source PDF; enables text-duplication detection")
    parser.add_argument('--no-render', action='store_true',
                        help="Skip the preview render step")
