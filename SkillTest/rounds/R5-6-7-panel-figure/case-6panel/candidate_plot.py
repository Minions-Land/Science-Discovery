...
def main():
    ...
    # figure-layout-defaults.md, 6-panel generalisation:
    # 4-cell hero region, subordinates in a contiguous filled remainder.
    ...
    plot_e(axes["E"])
    plot_f(axes["F"])
    axes["A"].set_title("Tumour volume reduction")
    for label, ax in axes.items():
        panel_label(ax, label)
    fig.savefig(OUT / "candidate.svg")
    fig.savefig(OUT / "candidate.pdf")
    fig.savefig(OUT / "candidate.png", dpi=300)
    plt.close(fig)
