...
def _extract_and_translate(insns, src_bottom, src_top, dx, dy, parent_ctm):
    ...
    for kind, a, b in _segment(insns):
        ...
        if ymin is None:
            ...
            if kind == "frame":
                out.append(chunk[0])
                out.extend(_extract_and_translate(chunk[1:-1], src_bottom,
                                                  src_top, dx, dy, list(ctm)))
                out.append(chunk[-1])
    return out
...
def main():
    ...
    print(f"Source: {SOURCE_PDF} ({src_w:.2f} × {src_h:.2f} pt)")
    ...
    filtered = _extract_and_translate(
        src_insns, SOURCE_REGION_BOTTOM, SOURCE_REGION_TOP,
        dx, dy, [1.0, 0, 0, 1.0, 0, 0],
    )
    ...
    if MASKS:
        ...
        parts.append("Q\n")
        pikepdf.Page(dst_page).contents_add(
            "".join(parts).encode("latin-1"), prepend=False)
